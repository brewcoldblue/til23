...
select user.email, board.createtime from user INNER JOIN board
ON user.name = board.writername where YEAR(board.createtime) < '2018';
...
ON user.name = board.writername GROUP BY YEAR(board.createtime);

-- #6. 2016년 이후 각 사용자들이 몇개의 board를 작성했는지 알 수 있도록 id와 작성한 board의 개수를 출력하시오.
select user.id , YEAR(createtime) as '개수'
from user INNER JOIN board
ON user.name = board.writername
where YEAR(createtime) > '2016' GROUP BY user.id;
...
SELECT board.title from user INNER JOIN board
ON user.name = board.writername
AND (age BETWEEN 30 AND 60);

-- #10. age가 60보다 큰 사람이 board를 작성한 적이 있다면 그 사람의 name을 출력하시오.
select DISTINCT user.name from user INNER JOIN board
ON user.name = board.writername
AND user.age > 60;
